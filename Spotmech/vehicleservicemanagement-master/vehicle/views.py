...
def approve_mechanic_view(request,pk):
    mechanicRating=forms.MechanicRatingForm()
    if request.method=='POST':
        mechanicRating=forms.MechanicRatingForm(request.POST)
        if mechanicRating.is_valid():
            mechanic=models.Mechanic.objects.get(id=pk)
            mechanic.rating=mechanicRating.cleaned_data['rating']
            mechanic.status=True
            mechanic.save()
        else:
            print("form is invalid")
        return HttpResponseRedirect('/admin-approve-mechanic')
    return render(request,'vehicle/admin_approve_mechanic_details.html',{'mechanicRating':mechanicRating})
...
def admin_add_mechanic_view(request):
    ...
    mechanicForm=forms.MechanicForm()
    mechanicRating=forms.MechanicRatingForm()
    mydict={'userForm':userForm,'mechanicForm':mechanicForm,'mechanicRating':mechanicRating}
    if request.method=='POST':
        userForm=forms.MechanicUserForm(request.POST)
        mechanicForm=forms.MechanicForm(request.POST,request.FILES)
        mechanicRating=forms.MechanicRatingForm(request.POST)
        if userForm.is_valid() and mechanicForm.is_valid() and mechanicRating.is_valid():
            user=userForm.save()
            user.set_password(user.password)
            user.save()
            mechanic=mechanicForm.save(commit=False)
...
@login_required(login_url='adminlogin')
def admin_view_mechanic_rating_view(request):
    mechanics=models.Mechanic.objects.all()
    return render(request,'vehicle/admin_view_mechanic_rating.html',{'mechanics':mechanics})
...
def update_rating_view(request,pk):
    ...
    if request.method=='POST':
        ...
        if mechanicRating.is_valid():
            ...
            print("form is invalid")
        return HttpResponseRedirect('/admin-view-mechanic-rating')
    return render(request,'vehicle/admin_approve_mechanic_details.html',{'mechanicRating':mechanicRating})
...
@login_required(login_url='mechaniclogin')
@user_passes_test(is_mechanic)
def mechanic_rating_view(request):
    mechanic=models.Mechanic.objects.get(user_id=request.user.id)
    workdone=models.Request.objects.all().filter(mechanic_id=mechanic.id).filter(Q(status="Repairing Done") | Q(status="Released"))
    return render(request,'vehicle/mechanic_rating.html',{'workdone':workdone,'mechanic':mechanic})
...
def user_mechanic_view(request):
    mechanics=models.Mechanic.objects.all()
    return render(request,'user_mechanicview.html',{'mechanics':mechanics})
